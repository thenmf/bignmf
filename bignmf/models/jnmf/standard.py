...
class StandardJnmf(JnmfBase):
    ...
    def update_weights(self):
        ...
        for key in self.x:
            ...
            denominator = denominator + np.dot(
                np.dot(self.w, self.h[key]), self.h[key].T
            )

        self.w = self.w * numerator / denominator

        for key in self.x:
            self.h[key] = np.multiply(
                self.h[key],
                np.divide(
                    np.dot(self.w.T, self.x[key]),
                    np.dot(self.w.T, np.dot(self.w, self.h[key]) + self.eps),
                ),
            )
        self.calc_error()
